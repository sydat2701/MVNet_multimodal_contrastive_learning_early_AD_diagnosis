...
class ExtractNetConTras(nn.Module):
    def __init__(self, contras_weight_path, fol_idx):
        super(ExtractNetConTras, self).__init__()
        self.contras_basex = get_contras_base_model(contras_weight_path, fol_idx, 'MRI')
        self.contras_basey = get_contras_base_model(contras_weight_path, fol_idx, 'fdg')
...
class MVNetContras(nn.Module):
    def __init__(self, contras_weight_path, fol_idx):
        ...
        self.extract_net = ExtractNetConTras(contras_weight_path, fol_idx)

        self.avg = nn.AdaptiveAvgPool3d(4)
        self.fc1 = nn.Linear(8*(2**3)*2, 32)
        self.lkrelu = nn.LeakyReLU(0.2)
        self.fc2 = nn.Linear(32, 1)
        self.sig = nn.Sigmoid()
        self.drop = nn.Dropout(0.4)
        self.drop1 = nn.Dropout(0.5)
        self.norm = nn.LayerNorm(8*(2**3)*2 ,eps=1e-6)
    ...
    def forward(self, x, y):
        featx, featy = self.extract_net(x, y)
