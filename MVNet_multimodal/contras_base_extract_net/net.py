...
class MVNet(nn.Module):
    def __init__(self):
        ...
        self.avg = nn.AdaptiveAvgPool3d(8)
        self.fc1 = nn.Linear((256*3+256)*(8**3), 128)

    def forward(self, x):
        featx, x = self.extract_net(x)
        return featx, x

import os
from glob import glob
def get_contras_base_model(contras_weight_path, fol_idx, modal):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = MVNet().to(device)
    weight_modal_path = os.path.join(contras_weight_path, fol_idx, modal)
    weight_path = os.path.join(weight_modal_path, glob(weight_modal_path+'/*.pth')[0])
